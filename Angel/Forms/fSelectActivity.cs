...
namespace Angel
{
    public partial class fSelectActivity : Form
    {
        ...
        public fSelectActivity(cAngel _angel)
        {
            ...
            for(int n = 0; n < 12; n++)
            {
                bool IsAct = fMain.oSchool.Activity[n].IsActive(NumDay, _Angel.Practice.OfActivityTotal(n));
                bnActivity[n].Visible = IsAct;
                bnActivity[n].Text = fMain.oSchool.Activity[n].Name;
                Tips.IsBalloon = true;
                Tips.SetToolTip(bnActivity[n], fMain.oSchool.Activity[n].Descr);
                if (IsAct == false)
                {
        ...
        private void bt_MouseHover(object sender, EventArgs e)
        {
            ...
            tbDescr.Text = fMain.oSchool.Activity[n].Descr;
        }
    }
}
