    ...
    public partial class fEditCharacteristic : Form
    {
        ...
        public fEditCharacteristic(object NumberCharacter)
        {
           InitializeComponent();
            ...
            lName.Text = fMain.oSchool.Activity[_Number].Name;

            for (int c = 0; c < _NumCharacter; c++)
            {
                ((Label)tlTrainPanel.GetControlFromPosition(0, c + 1)).Text = fMain.oSchool.CharacterInfo[c].Name + " (" + fMain.oSchool.CharacterInfo[c].NameAlt + ")";
                if (c == 6 && fMain.oSchool.Activity[_Number].Type != 0)  //Если это БП, то показывать только для тренировок...
                ...
                for (int d = 0; d < 7; d++)
                {
                    ...
                    PP[c, d].Enabled = (fMain.oSchool.Activity[_Number].Type == 0) ? (d < 3) : ((fMain.oSchool.Activity[_Number].WeekDayEnable[d] == 1) && c != 6);
                    if (tlTrainPanel.GetControlFromPosition(d + 1, c + 1) is PointsPanel)
                    {
                        ((PointsPanel)PP[c, d]).Points = cFunc.ArrToStr(fMain.oSchool.Activity[_Number].Growth[c][d]);
                    }
                    else if (tlTrainPanel.GetControlFromPosition(d + 1, c + 1) is CHSelectPanel)
                    {
                        ((CHSelectPanel)PP[c, d]).Points = int.Parse((cFunc.ArrToStr(fMain.oSchool.Activity[_Number].Growth[c][d])));
                    }
            ...
            for (int d = 0; d < 7; d++)
            {
                ...
                L.Text    = (fMain.oSchool.Activity[_Number].Type == 0) ? cFunc.ToRoman(d + 1) : cFunc.WeekDay[d].Substring(0, 4).ToUpper();
                L.Enabled = (fMain.oSchool.Activity[_Number].Type == 0) ? (d < 3) : (fMain.oSchool.Activity[_Number].WeekDayEnable[d] == 1);
                if (fMain.oSchool.Activity[_Number].Type == 0)
                {
                    if (!(d < 3))
                    {
                ...
                {
                    if (!(fMain.oSchool.Activity[_Number].WeekDayEnable[d] == 1))
                    {
                        tlTrainPanel.ColumnStyles[d + 1].SizeType = SizeType.Absolute;
                        tlTrainPanel.ColumnStyles[d + 1].Width = _DisabledColumnWidth;
                    }
