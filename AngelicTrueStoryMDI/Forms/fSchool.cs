...
namespace AngelicTrueStory
{
    public partial class fSchool : Form
    {
        public fSchool()
        {
            ...
            for (int i = 0; i < cSchool.Activity.Count(); i++)
            {
                Button TB = (Button)this.lpActivityDayEnabled.GetControlFromPosition(1, i + 1);
                TB.Text = cSchool.Activity[i].Name;
                for (int j = 0; j < cSchool.Activity[i].WeekDayEnable.Count(); j++)
                {
                    DayEnablePanel DEP = (DayEnablePanel)this.lpActivityDayEnabled.GetControlFromPosition(j + 2, i + 1);
                    DEP.CharacterChecked = (cSchool.Activity[i].WeekDayEnable[j] == 1);
                }
            }
        ...
        {
            for (int l = 0; l < cSchool.Activity.Count(); l++)
            {
                for (int d = 0; d < cSchool.Activity[l].WeekDayEnable.Count(); d++)
                {
                    bool _ShowStat = false;
                    DayEnablePanel D = ((DayEnablePanel)this.lpActivityDayEnabled.GetControlFromPosition(d + 2, l + 1));
                    D.LabelTop = "";
                    D.LabelBottom = "";
                    if (cSchool.Activity[l].WeekDayEnable[d] == 1)
                    {
                        int _Max = -10000;
                        int _CharacterID = -1;
                        for (int c = 0; c < cSchool.Activity[l].Growth.Length; c++)
                        {
                            for (int p = 0; p < cSchool.Activity[l].Growth[c][d].Length; p++)
                            {
                                if (cSchool.Activity[l].Growth[c][d][p] > _Max)
                                {
                                    _Max = cSchool.Activity[l].Growth[c][d][p];
                                    _CharacterID = c;
                                    _ShowStat = (_Max > 0);
                                }
                            }
                        ...
                        D.LabelBottom = cSchool.CharacterInfo[_CharacterID].Name;
                    }
